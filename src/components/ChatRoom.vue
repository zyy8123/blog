    ...
    >
      ...
      <div class="header">
        <img
          width="32"
          height="32"
          src="https://www.static.heyzyy.com/config/logo.png"
        />
      ...
      <div class="message" id="message">
        ...
        <!--
          isMyMessage：比对消息列表中的消息是否是本用户发出的，根据IP比较
        -->
        <div
...
export default {
  ...
  // 实例销毁前调用，关闭聊天窗口时，调用
  beforeDestroy() {
    clearInterval(this.heartBeat); // 清除掉超时时间
  },
  data: function() {
    return {
      isEmoji: false, // 控制表情窗口，添加完一个表情后关闭窗口
      isShow: false, // 是否打开or关闭聊天窗口
      ...
      count: 0, // 在线人数
      unreadCount: 0, // 未读消息
      isVoice: false, // 是否是语音，控制语音窗口
      voiceActive: false, // 录音是否开始
      startVoiceTime: null, // 语音长度
      WebsocketMessage: {
        // 发送的websocket消息
        type: null, // 消息类型
        data: null // 消息内容
  ...
  methods: {
    ...
    open() {
      ...
      this.isShow = !this.isShow; // 控制聊天窗口打开状态
    },
    // 打开表情窗口
    openEmoji() {
      this.isEmoji = !this.isEmoji; // 控制表情窗口打开状态
      this.isVoice = false; // 语音窗口关闭
    },
    // websocket 连接
    connect() {
      var that = this;
      // 从博客信息中取出 websocket 地址
        ...
        switch (data.type) {
          ...
          case 2: // 历史记录
            ...
            that.chatRecordList.forEach(item => {
              if (item.type == 5) {
                // 如果是语音消息
                that.voiceList.push(item.id); // 放入到语音列表中
              }
            });
            that.ipAddress = data.data.ipAddress; // ip
            that.ipSource = data.data.ipSource; // 属地
            break;
          case 3: // 文字消息
            that.chatRecordList.push(data.data);
            if (!that.isShow) {
              // 如果聊天窗口是关着的
              that.unreadCount++; // 未读数量+1
            }
            break;
          case 4: // 撤回
            // 是语音消息
    ...
    saveMessage(e) {
      ...
      var socketMsg = {
        ...
        content: this.content, // 消息
        userId: this.userId, // 用户Id
        type: 3, // 发送消息状态
        ipAddress: this.ipAddress, // ip地址
        ipSource: this.ipSource // ip属地
      };
      // 前端直接添加
    ...
    // 添加表情
    addEmoji(key) {
      this.isEmoji = false; // 一个只能添加一个标签
      this.$refs.chatInput.focus();
      this.content += key; // 消息贴上表情
    },
    // 展示菜单
    ...
    back(item, index) {
      var socketMsg = {
        // 构造socket消息
        id: item.id, // 消息Id
        isVoice: item.type == 5 // 判断是否是语音消息
      };
      this.WebsocketMessage.type = 4; // 撤回消息类型
      this.WebsocketMessage.data = socketMsg;
      // 发送给服务端，添加到数据库中
      this.websocket.send(JSON.stringify(this.WebsocketMessage));
  ...
  computed: {
    ...
    // 显示消息，在右(本用户发送的消息)，在左(其他用户发送的消息)，根据IP来判断
    isleft() {
      return function(item) {
    ...
    // 获取昵称，如果登录，显示昵称；没有登录，显示ip
    nickname() {
      return this.$store.state.nickname != null
        ? this.$store.state.nickname
        : this.ipAddress;
    },
    // 获取头像，如果登录，显示昵称；没有登录，显示ip
    avatar() {
